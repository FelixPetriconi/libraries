...
#include <stlab/algorithm/lower_bound.hpp>
...
inline I binary_search(I f, I l, const T& x, C c, P p) {
    I result = stlab::lower_bound(f, l, x, c, p);
...
takes an optional comparision function and uses \c stlab::less() if not provided.
...
inline I binary_search(I f, I l, const T& x, C c) {
    return stlab::binary_search(f, l, x, c, identity<T>());
}
...
inline typename boost::range_iterator<I>::type binary_search(I& range, const T& x) {
    return stlab::binary_search(boost::begin(range), boost::end(range), x);
}
...
inline typename boost::range_iterator<I>::type binary_search(I& range, const T& x, C c) {
    return stlab::binary_search(boost::begin(range), boost::end(range), x, c);
}
...
binary_search(I& r, const T& x, C c, P p) {
    return stlab::binary_search(boost::begin(r), boost::end(r), x, c, p);
}
