...
-> std::enable_if_t<has_process_notification_v<unwrap_reference_t<T>>, avoid<U>> {
    return unwrap(*x).notification();
}
...
template <typename F>
using notification_t = typename notification_impl<unwrap_reference_t<F>>::type;
...
struct shared_process
    ...
    */
    ...
    template <typename V>
    auto step() -> std::enable_if_t<has_process_yield_v<unwrap_reference_t<V>>> {
...
struct channel_combiner {
    ...
    static auto merge_helper(Executor&& executor, F&& f, UpstreamReceiver&&... upstream_receiver) {
        ...
        auto merge_process =
            std::make_shared<shared_process<typename M::template strategy_type<UpstreamReceiver...>, F, result_t, notif_t, 
                                            receiver_t<UpstreamReceiver>...>>(
...
template <typename E, typename T, typename Notification>
struct channel_ {
    static auto create(E executor) {
        auto p = std::make_shared<
            detail::shared_process<detail::default_queue_strategy<T>, identity, T, Notification, T>>(
...
template <typename E, typename Notification>
struct channel_<E, void, Notification> {
    static auto create(E executor) {
        auto p = std::make_shared<
            detail::shared_process<detail::default_queue_strategy<void>, identity, void, Notification, void>>(
            std::move(executor), identity());

        return receiver<void, Notification>(p);
    }
};
...
auto channel(Executor executor) {
    return detail::channel_<Executor, T, Notification>::create(std::move(executor));
}
...
public:
    ...
    auto operator|(F&& f) const {
        ...
        auto p = std::make_shared<detail::shared_process<detail::default_queue_strategy<T>, F,
                                                         detail::yield_type<unwrap_reference_t<F>, T>, detail::notification_t<F>, T>>(
            _p->executor(), std::forward<F>(f), _p);
        _p->map(sender<T>(p));
        return receiver<detail::yield_type<unwrap_reference_t<F>, T>, detail::notification_t<F>>(std::move(p));
    }
